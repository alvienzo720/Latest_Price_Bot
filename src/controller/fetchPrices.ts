...
export const fetchPrices = async () => {
  try {
    const response = await axios.get(BotParams.API_URL, {
      headers: { "X-CMC_PRO_API_KEY": BotParams.API_KEY },
    ...
    if (Object.keys(previousPrices).length !== 0) {
      for (const symbol of symboList) {
        ...
        if (increase >= 5 && symbol === "BTC") {
          console.log(
            `${symbol} Price Increased by $${increase}, You can Sell Now! 🚀 `
          );
          sendMessage(
            `⬆️ ${symbol} Price Increased by $${increase}, You can Sell Now! 🚀 `
          );
          ...
          console.log(
            `${symbol} Price Dropped by $${increase} You can Buy Now!`
          );
          sendMessage(
            `🔻${symbol} Price Dropped by $${increase} You can Buy Now!`
          );
